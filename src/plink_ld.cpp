...
static void EmitRow(DataChunk &output, idx_t row_idx, const PlinkLdBindData &bind_data,
	...
	for (idx_t out_col = 0; out_col < gstate.column_ids.size(); out_col++) {
		...
		switch (file_col) {
		case COL_CHROM_A:
			CompatFlatDataMutable<string_t>(vec)[row_idx] = StringVector::AddString(vec, variants.GetChrom(vidx_a));
			break;
		case COL_POS_A:
			CompatFlatDataMutable<int32_t>(vec)[row_idx] = variants.GetPos(vidx_a);
			break;
		case COL_ID_A: {
			...
			if (id_a.empty()) {
				...
				CompatFlatDataMutable<string_t>(vec)[row_idx] = StringVector::AddString(vec, id_a);
			}
			break;
		}
		case COL_CHROM_B:
			CompatFlatDataMutable<string_t>(vec)[row_idx] = StringVector::AddString(vec, variants.GetChrom(vidx_b));
			break;
		case COL_POS_B:
			CompatFlatDataMutable<int32_t>(vec)[row_idx] = variants.GetPos(vidx_b);
			break;
		case COL_ID_B: {
			...
			if (id_b.empty()) {
				...
				CompatFlatDataMutable<string_t>(vec)[row_idx] = StringVector::AddString(vec, id_b);
			}
			break;
		}
		case COL_R:
			if (result.is_valid) {
				CompatFlatDataMutable<double>(vec)[row_idx] = result.r;
			} else {
				FlatVector::SetNull(vec, row_idx, true);
			}
			break;
		case COL_R2:
			if (result.is_valid) {
				CompatFlatDataMutable<double>(vec)[row_idx] = result.r2;
			} else {
				FlatVector::SetNull(vec, row_idx, true);
			}
			break;
		case COL_D_PRIME:
			if (result.is_valid) {
				CompatFlatDataMutable<double>(vec)[row_idx] = result.d_prime;
			} else {
				FlatVector::SetNull(vec, row_idx, true);
			}
			break;
		case COL_OBS_CT:
			CompatFlatDataMutable<int32_t>(vec)[row_idx] = static_cast<int32_t>(result.obs_ct);
			break;
		default:
			break;
		}
