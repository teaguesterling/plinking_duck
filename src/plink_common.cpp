...
// ---------------------------------------------------------------------------
// Per-thread pgenlib context initialization
// ---------------------------------------------------------------------------
...
void InitPgenThreadContext(PgenThreadContext &ctx, const string &pgen_path, uint32_t raw_variant_ct,
	...
	plink2::PglErr err = plink2::PgfiInitPhase1(pgen_path.c_str(), nullptr, raw_variant_ct, raw_sample_ct, &header_ctrl,
	                                            &ctx.pgfi, &pgfi_alloc_cacheline_ct, errstr_buf);
	if (err != plink2::kPglRetSuccess) {
		throw IOException("PgenThreadContext: phase 1 init failed: %s", errstr_buf);
	}

	if (pgfi_alloc_cacheline_ct > 0) {
		ctx.pgfi_alloc.Allocate(pgfi_alloc_cacheline_ct * plink2::kCacheline);
	}

	uint32_t max_vrec_width = 0;
	uintptr_t pgr_alloc_cacheline_ct = 0;

	err = plink2::PgfiInitPhase2(header_ctrl, 0, 0, 0, 0, ctx.pgfi.raw_variant_ct, &max_vrec_width, &ctx.pgfi,
	                             ctx.pgfi_alloc.As<unsigned char>(), &pgr_alloc_cacheline_ct, errstr_buf);
	if (err != plink2::kPglRetSuccess) {
		plink2::PglErr cleanup_err = plink2::kPglRetSuccess;
		plink2::CleanupPgfi(&ctx.pgfi, &cleanup_err);
		throw IOException("PgenThreadContext: phase 2 init failed: %s", errstr_buf);
	}

	if (pgr_alloc_cacheline_ct > 0) {
		ctx.pgr_alloc.Allocate(pgr_alloc_cacheline_ct * plink2::kCacheline);
	}

	err = plink2::PgrInit(pgen_path.c_str(), max_vrec_width, &ctx.pgfi, &ctx.pgr,
	                      ctx.pgr_alloc.As<unsigned char>());
	if (err != plink2::kPglRetSuccess) {
		plink2::PglErr cleanup_err = plink2::kPglRetSuccess;
		plink2::CleanupPgr(&ctx.pgr, &cleanup_err);
		cleanup_err = plink2::kPglRetSuccess;
		plink2::CleanupPgfi(&ctx.pgfi, &cleanup_err);
		throw IOException("PgenThreadContext: PgrInit failed for '%s'", pgen_path);
	}

	if (sample_subset) {
		plink2::PgrSetSampleSubsetIndex(sample_subset->CumulativePopcounts(), &ctx.pgr, &ctx.pssi);
	} else {
		plink2::PgrClearSampleSubsetIndex(&ctx.pgr, &ctx.pssi);
	}
