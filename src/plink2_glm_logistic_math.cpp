...
#ifdef FVEC_32
static inline void MultMatrixDxnVectNF(const float *mm, const float *vect, uint32_t col_ct, uint32_t row_ct,
...
static inline void ComputeTwoDiagTripleProductF(const float *aa, const float *bb, const float *vv, uint32_t nn,
                                                float *__restrict raa_ptr, float *__restrict rab_ptr,
                                                float *__restrict rbb_ptr) {
...
static inline void ComputeThreeTripleProductF(const float *bb, const float *a1, const float *a2, const float *a3,
                                              const float *vv, uint32_t nn, float *__restrict r1_ptr,
                                              float *__restrict r2_ptr, float *__restrict r3_ptr) {
...
static inline void ComputeTwoPlusOneTripleProductF(const float *bb, const float *a1, const float *a2, const float *vv,
                                                   uint32_t nn, float *__restrict r1_ptr, float *__restrict r2_ptr,
                                                   float *__restrict r3_ptr) {
	VecF s1 = vecf_setzero();
	VecF s2 = vecf_setzero();
	VecF s3 = vecf_setzero();
	for (uint32_t uii = 0; uii < nn; uii += kFloatPerFVec) {
...
void ComputeHessianF(const float *mm, const float *vv, uint32_t col_ct, uint32_t row_ct, float *__restrict dest) {
	...
	if (row_ct > 3) {
		...
		for (uint32_t row_idx = 0; row_idx < row_ctm3; row_idx += 3) {
			...
			ComputeTwoDiagTripleProductF(mm_cur, &(mm_cur[col_ctav]), vv, col_ct, &(dest[row_idx * row_ctavp1]),
			                             &(dest[(row_idx + 1) * row_ctavp1 - 1]), &(dest[(row_idx + 1) * row_ctavp1]));
			ComputeTwoPlusOneTripleProductF(
			    &(mm_cur[2 * col_ctav]), &(mm_cur[col_ctav]), mm_cur, vv, col_ct, &(dest[(row_idx + 2) * row_ctavp1]),
			    &(dest[(row_idx + 2) * row_ctavp1 - 1]), &(dest[(row_idx + 2) * row_ctavp1 - 2]));
			for (uint32_t row_idx2 = row_idx + 3; row_idx2 != row_ct; ++row_idx2) {
				ComputeThreeTripleProductF(&(mm[row_idx2 * col_ctav]), mm_cur, &(mm_cur[col_ctav]),
				                           &(mm_cur[2 * col_ctav]), vv, col_ct, &(dest[row_idx2 * row_ctav + row_idx]),
				                           &(dest[row_idx2 * row_ctav + row_idx + 1]),
				                           &(dest[row_idx2 * row_ctav + row_idx + 2]));
			}
		}
	}
	switch (row_ct % 3) {
	case 0:
		ComputeTwoPlusOneTripleProductF(&(mm[(row_ct - 3) * col_ctav]), &(mm[(row_ct - 2) * col_ctav]),
		                                &(mm[(row_ct - 1) * col_ctav]), vv, col_ct, &(dest[(row_ct - 3) * row_ctavp1]),
		                                &(dest[(row_ct - 2) * row_ctavp1 - 1]), &(dest[(row_ct - 1) * row_ctavp1 - 2]));
		// fall through
	case 2:
		ComputeTwoDiagTripleProductF(&(mm[(row_ct - 2) * col_ctav]), &(mm[(row_ct - 1) * col_ctav]), vv, col_ct,
		                             &(dest[(row_ct - 2) * row_ctavp1]), &(dest[(row_ct - 1) * row_ctavp1 - 1]),
		                             &(dest[(row_ct - 1) * row_ctavp1]));
		break;
	case 1:
		dest[(row_ct - 1) * row_ctavp1] =
		    TripleProductF(&(mm[(row_ct - 1) * col_ctav]), &(mm[(row_ct - 1) * col_ctav]), vv, col_ct);
	}
}
...
BoolErr LogisticRegressionF(const float *yy, const float *xx, const float *sample_offsets, uint32_t sample_ct,
	...
	for (uint32_t iteration = 0;; ++iteration) {
		...
		if (iteration > 3) {
			if (((delta_coef > S_CAST(float, 20.0)) && (delta_coef > 2 * min_delta_coef)) ||
			    ((iteration > 6) && (fabsf(S_CAST(float, 1.0) - delta_coef) < S_CAST(float, 1e-3)))) {
				return 1;
			}
			if (iteration > 13) {
				for (uint32_t pred_idx = 0; pred_idx != predictor_ct; ++pred_idx) {
...
static void FirthComputeHdiagWeightsF(const float *yy, const float *xx, const float *pp, const float *hh,
                                      const float *vv, uint32_t predictor_ct, uint32_t predictor_ctav,
                                      uint32_t sample_ct, uint32_t sample_ctav, float *hdiag, float *ww,
                                      float *tmpnxk_buf) {
	ColMajorFmatrixMultiplyStrided(xx, hh, sample_ct, sample_ctav, predictor_ct, predictor_ctav, predictor_ct,
	                               sample_ctav, tmpnxk_buf);
#ifdef __LP64__
...
static void FirthComputeSecondWeightsF(const float *hdiag, const float *vv, __maybe_unused uint32_t sample_ct,
                                       __maybe_unused uint32_t sample_ctav, float *ww) {
#ifdef __LP64__
	const VecF one = VCONST_F(1.0);
	for (uint32_t sample_offset = 0; sample_offset < sample_ctav; sample_offset += kFloatPerFVec) {
...
BoolErr FirthRegressionF(const float *yy, const float *xx, const float *sample_offsets, uint32_t sample_ct,
                         ...
                         float *vv, float *ustar, float *delta, float *hdiag, float *ww, float *hh0,
                         float *tmpnxk_buf) {
	const uintptr_t predictor_ctav = RoundUpPow2(predictor_ct, kFloatPerFVec);
	const uintptr_t sample_ctav = RoundUpPow2(sample_ct, kFloatPerFVec);
	...
	for (uint32_t iter_idx = 0;; ++iter_idx) {
		...
		if (InvertSymmdefFmatrixFirstHalf(predictor_ct, predictor_ctav, hh0, half_inverted_buf, inv_1d_buf,
		                                  dbl_2d_buf)) {
			return 1;
		}
		...
		FirthComputeHdiagWeightsF(yy, xx, pp, hh0, vv, predictor_ct, predictor_ctav, sample_ct, sample_ctav, hdiag, ww,
		                          tmpnxk_buf);
		...
		if (InvertSymmdefFmatrixFirstHalf(predictor_ct, predictor_ctav, hh, half_inverted_buf, inv_1d_buf,
		                                  dbl_2d_buf)) {
			return 1;
		}
