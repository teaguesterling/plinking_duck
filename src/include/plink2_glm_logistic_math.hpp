...
uint32_t GenoarrToFloatsRemoveMissing(const uintptr_t* genoarr, const float* __restrict table,
                                       uint32_t sample_ct, float* __restrict dst);
...
BoolErr LogisticRegressionF(const float* yy, const float* xx, const float* sample_offsets,
                            uint32_t sample_ct, uint32_t predictor_ct, float* __restrict coef,
                            uint32_t* is_unfinished_ptr, float* __restrict ll,
                            float* __restrict pp, float* __restrict vv,
                            float* __restrict hh, float* __restrict grad,
                            float* __restrict dcoef);
...
BoolErr FirthRegressionF(const float* yy, const float* xx, const float* sample_offsets,
                         uint32_t sample_ct, uint32_t predictor_ct, float* beta,
                         uint32_t* is_unfinished_ptr, float* hh,
                         double* half_inverted_buf, MatrixInvertBuf1* inv_1d_buf,
                         double* dbl_2d_buf, float* pp, float* vv,
                         float* ustar, float* delta, float* hdiag, float* ww,
                         float* hh0, float* tmpnxk_buf);

// Hessian computation: hh = mm * diag(vv) * mm^T (float, covariate-major layout).
void ComputeHessianF(const float* mm, const float* vv, uint32_t col_ct, uint32_t row_ct,
...
void SolveLinearSystemF(const float* ll, const float* yy, uint32_t predictor_ct,
                        float* __restrict xx);

// Check log-likelihood for convergence issues (returns 1 if any p==0 or p==1).
BoolErr ComputeLoglikCheckedF(const float* yy, const float* pp, uint32_t sample_ct,
                              double* loglik_ptr);
