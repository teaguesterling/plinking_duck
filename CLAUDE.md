...
- `read_pfile`/`read_pvar` first positional arg accepts a single prefix `VARCHAR` **or** a `LIST(VARCHAR)` of prefixes/paths (mirrors `read_csv`/`read_json`); a list row-concatenates variants in file order. All files in a list must share the **same sample set (same IIDs, same order)** — the variant-sharded, identical-`.psam` layout; alignment is **positional** (trust-the-caller, no per-file identity check), but a mismatched `sample_ct` across files is a hard error. Phase 1: `variant` + `genotype` orient row-concat across files; `orient := 'sample'` with **multiple** files is not yet supported (single file works). Named `pgen`/`pvar`/`psam` overrides are single-file only (error if combined with a multi-element list). `read_pgen` multi-file input is planned (not yet available). Pairs with the scalarfs `pathmacro:` protocol (a catalog macro returns a `VARCHAR[]` of shard prefixes that feeds `read_pfile([...])`).
